...
//  int maxAE = 0;
  ...
  public void setBrushSizes(int minSize, int numSizes, float sizeRatio) {
    ...
    for(int i = 0; i < numSizes; i++) {
      brushes[numSizes-i-1] = int(curBrushSize);
  ...
  public color colorJitterize(color c) {
    int red = int(random(red(c)-jitter, red(c)+jitter));
    int green = int(random(green(c)-jitter, green(c)+jitter));
    int blue = int(random(blue(c)-jitter, blue(c)+jitter));
  ...
  public PImage convolution(PImage srcImg, int convSize){
    ...
    float v = (float) (1.0 / (kernDim*kernDim));
...
class Stroke {
    ...
    public float luminance(int col) {
      return (float) (0.30 * red(col) + .59 * green(col) + 0.11 * blue(col));
    }
    /**
